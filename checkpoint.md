...
4. **Propagate into IaC (this repo)** — the nuke/deploy must reproduce the working box:
   - `ansible/files/configure-ai-engine-inside-llc.sh` → `configure-ai-engine-inside-lxc.sh`: correct default model, unit flags (HSA 11.0.0, `VLLM_USE_V2_MODEL_RUNNER=0`, `--gpu-memory-utilization 0.25`), and make the 4 venv shims/patches idempotent (drop shim+.pth files, run the same python patch scripts).
   - `deploy-hlh-ai-engine-vllm.sh`: default model + HSA override (LXC_ID=113, LXC_NAME=hlh-ai-engine-vllm, IP 192.168.1.13/24, POOL RaidZ1-6TB, models bind host `/srv/ai/models` → LXC `/srv/ai/models`).
